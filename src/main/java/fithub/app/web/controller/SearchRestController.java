...
public class SearchRestController {
    ...
    @Operation(summary = "게시글 검색 API - 최신순 ", description = "tag에 검색 태그를 담아서 전달, last로 페이징 🐶 전체 검색은 이 api와 운동인증 검색 api 2개를 같이 사용해주세요🐶")
    @ApiResponses({
            @ApiResponse(responseCode = "2000", description = "OK : 검색결과 있음"),
            @ApiResponse(responseCode = "2021", description = "OK : 검색결과 없음"),
    ...
    @Operation(summary = "게시글 검색 API - 인기순 ", description = "tag에 검색 태그를 담아서 전달, last로 페이징 🐶 전체 검색은 이 api와 운동인증 검색 api 2개를 같이 사용해주세요🐶")
    @ApiResponses({
            @ApiResponse(responseCode = "2000", description = "OK : 검색결과 있음"),
            @ApiResponse(responseCode = "2021", description = "OK : 검색결과 없음"),
            @ApiResponse(responseCode = "5000", description = "Server Error : 똘이에게 알려주세요",content =@Content(schema =  @Schema(implementation = ResponseDto.class)))
    ...
    @Operation(summary = "인증 검색 API - 인기순 ", description = "tag에 검색 태그를 담아서 전달, last로 페이징")
    @ApiResponses({
            @ApiResponse(responseCode = "2000", description = "OK : 정상응답"),
            @ApiResponse(responseCode = "2021", description = "OK : 검색결과 없음"),
            @ApiResponse(responseCode = "5000", description = "Server Error : 똘이에게 알려주세요",content =@Content(schema =  @Schema(implementation = ResponseDto.class)))
